...
def test_summarise_tcp_sets_protocol_and_ports() -> None:
    packet = Ether() / IP(src="1.2.3.4", dst="5.6.7.8") / TCP(sport=12345, dport=80)
...
def test_summarise_udp_sets_correct_protocol() -> None:
    packet = Ether() / IP(src="10.0.0.1", dst="10.0.0.2") / UDP(sport=5000, dport=53)
    packet.time = 1_700_000_000.0
    summary = summarise_packet(packet)
...
def test_summarise_dns_extracts_query_name() -> None:
    packet = Ether() / IP() / UDP() / DNS(qd=DNSQR(qname="evil.example.com"))
    packet.time = 1_700_000_000.0
    summary = summarise_packet(packet)
...
def test_summarise_arp_sets_ips() -> None:
    packet = Ether() / ARP(psrc="192.168.1.1", pdst="192.168.1.2")
    packet.time = 1_700_000_000.0
    summary = summarise_packet(packet)
...
def test_summarise_ipv6_packet() -> None:
    packet = Ether() / IPv6(src="::1", dst="::2") / TCP()
    packet.time = 1_700_000_000.0
    summary = summarise_packet(packet)
...
def test_summary_string_contains_ips_and_protocol() -> None:
    packet = Ether() / IP(src="1.1.1.1", dst="2.2.2.2") / TCP(sport=100, dport=200)
    packet.time = 1_700_000_000.0
    summary = summarise_packet(packet)
