...
def test_extract_ip_ipv4() -> None:
    packet = stamped(Ether() / IP(src="1.2.3.4", dst="5.6.7.8") / TCP())
    src, dst = extract_ip_addresses(packet)
    assert src == "1.2.3.4"
    assert dst == "5.6.7.8"


def test_extract_ip_ipv6() -> None:
    packet = stamped(Ether() / IPv6(src="::1", dst="::2") / TCP())
    src, dst = extract_ip_addresses(packet)
    assert src == "::1"
    assert dst == "::2"
...
def test_extract_ip_arp() -> None:
    packet = stamped(Ether() / ARP(psrc="10.0.0.1", pdst="10.0.0.2"))
    src, dst = extract_ip_addresses(packet)
    assert src == "10.0.0.1"
    assert dst == "10.0.0.2"
